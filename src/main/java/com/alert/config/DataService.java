...
public class DataService {
    ...
    public Integer[] validate() {
        ...
        try {
            ...
            preparedStatement = connection.prepareStatement("SELECT SUM(t.`push`) AS push ,SUM(t.`view`) AS 'view' ,SUM(t.`click`) AS click,SUM(t.`d_oth`+t.`d_wall`) AS download,SUM(t.`i_oth`+t.`i_wall`) AS 'install' FROM ad_hour_report_new t WHERE t.`hour` LIKE '" + day + "%'");
            resultSet = preparedStatement.executeQuery();
            while (resultSet.next()) {
    ...
    public void sendMail() {
        ...
        Integer[] size = dataService.validate();
        StringBuilder sb = new StringBuilder();
        sb.append(TimeUtil.getDayLastHour(new Date())).append("小时统计结果：").append(dataService.hasdata());
